...
const Card = ({ task }: CardProps) => {
    ...
    const handleChange = (status: string) => {
        setEditableTask((prev) => ({ ...prev, status }));
    };
                ...
                <div className="fixed inset-0 flex items-center justify-center z-50 bg-gray-900 bg-opacity-50">
                    <div className="bg-black/30 backdrop-blur-md rounded-lg p-6 w-96 shadow-lg">
                        ...
                        <div className="relative">
                            ...
                            {statusDropdownOpen && (
                                <ul className="absolute w-full mt-2 bg-black/20 backdrop-blur-md rounded-lg">
                                    <li className="py-2 px-4 hover:bg-gray-100/30" onClick={() => handleChange("New")}>
                                        <span className="px-2 py-1 rounded-full bg-gray-400 text-white">New</span>
                                    </li>
                                    <li
                                        className="py-2 px-4 hover:bg-gray-100/30"
                                        onClick={() => handleChange("Inprogress")}
                                    >
                                        <span className="px-2 py-1 rounded-full bg-blue-500 text-white">In Progress</span>
                                    </li>
                                    <li
                                        className="py-2 px-4 hover:bg-gray-100/30"
                                        onClick={() => handleChange("Complete")}
                                    >
                                        <span className="px-2 py-1 rounded-full bg-green-500 text-white">Complete</span>
                                    </li>
                                </ul>
                            )}
                        </div>
                        ...
                        <p className="mt-2 text-white">
                            <span className="font-medium text-white">From:  {editableTask.formToDate[0]} </span><br />
